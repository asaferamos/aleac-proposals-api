...
RSpec.describe BillsController, type: :request do
    ...
    it "test some fields of proposal" do
        ...
        expect(json_response['authors']).to eq(["Governo - Governador","Teste"])
        expect(json_response['kind']).to eq("Projeto de Lei")
        expect(json_response['year']).to eq("2013")
        expect(json_response['link']).to eq(
            "#{Rails.configuration.url_aleac}/media/sapl/public/materialegislativa/2013/4059/4059_texto_integral.pdf"
        )
